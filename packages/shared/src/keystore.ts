...
export function requirePublicInfo(id?: string): { publicKey: string; address: string } {
  const info = getPublicInfo(id)
  if (!info) {
    throw new Error('No wallet found. Run "ow wallet create" or "ow wallet import" first.')
  }
  return info
...
export function unlockKeypair(password: string, id?: string) {
  const ks = loadKeystore(password, id)
